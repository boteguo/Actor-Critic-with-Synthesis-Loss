...
def run(args):
    ...
    path = f"new_result/{args.agentname}_{args.env}_{args.seed}{args.special}"
    ...
    R,max_R,R_Q = agent.evaluate()
    evaluations = [R]
    max_evaluations = [max_R]
    minerq = [R_Q]
    if args.clamp_q:
        ...
        delta_t = torch.sigmoid(delta.detach().cpu())/2 +0.5
        lambd_t = args.expscalar * torch.exp(log_lambd.detach().cpu())
        delta_t = delta_t.numpy()
        lambd_t = lambd_t.numpy()
        DELTA = [delta_t]
        LAMBD = [lambd_t]
    else:
        log_lambd = -1
    ...
    for i in range(args.num_steps):
        ...
        if total_numsteps % args.eval_interval == 0:
            ...
            if args.clamp_q:
                ...
                LAMBD.append(lambd_t)
                np.save(f"{path}/lambd",LAMBD)
                np.save(f"{path}/delta",DELTA)
            if args.save_model:
