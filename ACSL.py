...
class ACSLAgent(object):
    def __init__(self, env, test_env,path,batch_size=64, lr=0.0003, 
                 ...
                 max_episode_steps=1000, cuda=True,args = 0,beta=0.001):
        self.args =args
        self.env = env
    ...
    def softmax_operator(self, q_vals, beta,noise_pdf=None):
    ...
    def calc_target_q(self, rewards, next_states, masks,delta):
        with torch.no_grad():
            if not self.args.softmax:
                ...
                next_states = next_states.repeat((1,50,1)) 
                next_action_mean = self.policy.sample_random_action(next_states)
                next_q1,next_q2 = self.critic_target(next_states, next_action_mean)
                if delta != -1:
                    next_q1 = (torch.sigmoid(delta)/2 +0.5) * torch.min(next_q1,next_q2)\
                    ...
                    next_q1 = torch.min(next_q1,next_q2)
                next_q1 = torch.squeeze(next_q1,2)
                next_q1 = self.softmax_operator(next_q1,self.beta)
    ...
    def calc_critic_loss(self, 
        ...
        if log_lambd != -1:
            if self.args.mode == "exp":
                loss1 =  expscalar * torch.exp(log_lambd) * curr_q1 + sigma1 ** 2
            else:
